...
def assign_mat(Scone):
	...
	if cmds.objExists('lambertA_mat'):
		...
		cmds.setAttr(('lambertA_mat.incandescence'), 1,0,0, type = 'double3')
		cmds.setAttr(('lambertA_mat.color'), 1,0,0, type = 'double3')
	
		
	else:

		myshade = cmds.shadingNode('lambert', name = 'lambertA_mat', asShader = True)
